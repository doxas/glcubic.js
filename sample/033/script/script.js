...
window.onload = function(){
	...
	// texture load
	gl3.create_texture('image/normal.png', 0, init);
};
...
function init(){
	...
	var prg = gl3.program.create(
		...
		[3, 3, 4, 2],
		['mMatrix', 'mvpMatrix', 'invMatrix', 'lightPosition', 'eyePosition', 'normalMapTexture'],
		['matrix4fv', 'matrix4fv', 'matrix4fv', '3fv', '3fv', '1i']
	);

	// sphere mesh
	var sphereData = gl3.mesh.sphere(64, 64, 1.0);
	var sphereVBO = [
	...
	function render(){
		...
		prg.push_shader([mMatrix, mvpMatrix, invMatrix, lightPosition, cameraPosition, 0]);
