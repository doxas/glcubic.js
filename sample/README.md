...
ここでは、深度の影響を受けることについて理解を深めることも大切にする。

またブレンドファクターについては次のサンプルに譲る。

### 026

ブレンドファクターについて理解を深める。

加算合成などの他のブレンド方法についても理解する。

### 027

カリングについてあらためて実装して体験してみる。

ここでのポイントは、ひとまず加算合成云々には触れずに、ひとまずカリングの有効化の方法や、表裏反転についてしっかり理解すること。

### 028

地球の大気を描くことにチャレンジする。

恐らく実装出来る人はいないが、一応そのままやってみてもらう。

図解してあげないときついかもしれない予想。

### 029

距離フォグを実装する。

フォグの開始距離や終了距離など、かなり数学的な知識を要する部分もあるのでイメージしやすいように。

最終的には色や開始地点をもっと柔軟に javascript から操作するための実装に挑戦してもらうなどするとよいかも。

### 030

トゥーンレンダリングを実装する。

テクスチャを利用して陰影付けを行うスタイルにして、プロシージャル陰影付けにはしない。
...
法線や、接線ベクトル、従法線ベクトルなどについてしっかり扱う。

正直、理解が追いつかない可能性が高いので図解する。

### 034

フレームバッファによるオフスクリーンレンダリング。

まずは単純にオフスクリーンのものを何かしらのモデルに貼り付けるのみ。

フレームバッファに関する数多の概念をまずはしっかり押さえる。また、ここではテクスチャ座標の上下反転に関する知識をしっかりみにつける。

### 035

オフスクリーンでネガポジ反転処理を行う。

フレームバッファを利用した作例のひとつとして、ネガポジ反転シェーダを組む。

ここでは、併せて正射影変換の使い方や利点、マルチシェーダについて言及する。特にマルチシェーダにおけるプログラムオブジェクト関連の処理移行がかなり最初はわかりにくいので、可能であればスライド内で箇条書き対応したほうがよい。

### 

射影テクスチャマッピング（光学迷彩の準備段階として）。

完成形はあくまでも光学迷彩として、その事前準備という名目で射影テクスチャマッピングを行う。

### 

光学迷彩を実装する。

キューブマップは使わずに、フレームバッファを利用して光学迷彩を実装する。

イメージ的には、方眼紙のようなテクスチャを焼きこんだオフスクリーンのレンダリング結果を利用することで、迷彩効果がわかりやすいようにする。

### 

キューブ環境マッピングの実装。

六面テクスチャを利用するところから解説。まずはスカイボックス。

※事前にテクスチャ準備したほうがいいかもしれん。

### 

屈折マッピング。

キューブ環境マッピングができればそれほど難しくないので、さらりと。

### 

動的キューブマップ。

スクール第一期のときのような、プロシージャルなものにするのがよいかも。
...
正射影投影によるシェーダエフェクト実装の基本として、まずはグレイスケールを実装する。

いつものテレビのやつと、単純に合成した場合との差について解説する。
...
floor 使ったお手軽バージョンを試す。

### 

ガウシアンブラーフィルタ。

ある意味でかなり様々なもののベースになるので、概念の部分を中心にしっかり扱う。

### 

簡易ノイズシェーダ。

ガッチリしたのはきついので、簡単なノイズシェーダを記述する。
...
ノイズを組み合わせて、ズームブラーを実装する。

### 

さらに応用で、ゴッドレイ実装する。

ゴッドレイの考え方がわかれば恐らくそれなりにできるはずなので、ある程度作業をセルフでやってもらう形式にするか？

### 

ソーベルフィルタによるエッジ検出。

どのような概念を用いてエッジを検出するのか、その仕組みをしっかり理解してもらう。

### 

ソーベルによるエッジ検出とガウシアンブラーの合わせ技シェーダ。

ネオンが光り輝くような、エッジ検出とブラーと加算合成のネオンシェーダを書く。
