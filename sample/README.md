...
ローカルの任意の画像を使ってテストできるようなサンプルにすることができれば最高。
...
ここではまず、行列による座標変換を使ってカメラを動かすところまでを作る。

ベースになるのは 012 のサンプルにする。


### 014

行列による座標変換ではうまくいかない部分が出てくることが理解できたら、ここでクォータニオンに簡単に触れつつ、それによってカメラの視点を動かすことができるサンプルを作成する。

とは言え、まずはこちらで作成したものをベースに体感してもらうところまで。


### 015

パーティクルシステムとカメラのクォータニオンによる変換が行えるようになったので、最後の仕上げに輝度の算出によりパーティクルの位置を調整するシステムに切り替え、色は点の色としてそのまま出せるようなサンプルを作る。

これが第二回の総仕上げ的なところになると予想。
