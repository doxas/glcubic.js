...
function init(){
	...
	// plane mesh
	var planeData = gl3.mesh.plane(5.0, 5.0, [1.0, 1.0, 1.0, 0.5]);
	var planeVBO = [
	...
	function render(){
		...
		gl3.mat4.translate(mMatrix, [1.0, 1.0, -0.5], mMatrix);
		gl3.mat4.multiply(vpMatrix, mMatrix, mvpMatrix);
		prg.push_shader([mvpMatrix, 0, 0, 1]);
		...
		gl3.mat4.translate(mMatrix, [-1.0, -1.0, 0.5], mMatrix);
		gl3.mat4.multiply(vpMatrix, mMatrix, mvpMatrix);
		prg.push_shader([mvpMatrix, 1, 0, 1]);
		gl3.draw_elements(gl3.gl.TRIANGLES, planeData.index.length);
